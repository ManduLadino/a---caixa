...
export default function RealisticStoneBox() {
  ...
  const [selectedStoneType, setSelectedStoneType] = useState<string>("ametista")
  ...
  const [boxStyle, setBoxStyle] = useState<"golden" | "wooden" | "dark">("golden")
  ...
  const [stoneCategories, setStoneCategories] = useState<string[]>([
    "Todas",
    "Cristalinas",
    "Protetoras",
    "Energéticas",
  ...
  // Gera uma pedra aleatória do tipo selecionado
  const generateStone = (x: number, y: number, stoneType: string = selectedStoneType) => {
    const stoneData = stonePrompts[stoneType as keyof typeof stonePrompts]
    if (!stoneData) return null

    const size = Math.floor(Math.random() * 15) + 20 // 20-35px
    const energy = stoneData.energyLevel + (Math.random() * 20 - 10) // Variação de energia
    ...
    return {
      ...
      energy,
      glowColor,
      texture: stoneData.texture,
      shape: stoneData.shape,
      finish: stoneData.finish,
      elevation,
      shadow,
      glow,
      opacity,
  ...
  // Adiciona uma pedra na posição especificada
    ...
    if (clickedStone) {
      ...
      if (e.shiftKey) {
        ...
        setSelectedStoneInfo({
          ...
          texture: stoneData?.texture || "Textura desconhecida",
  ...
  const randomizeStones = () => {
    ...
    // Lista de todos os tipos de pedras
    const stoneTypes = Object.keys(stonePrompts)

    // Gera exatamente 33 pedras em diferentes padrões
    for (let i = 0; i < 33; i++) {
      const stoneType = stoneTypes[i % stoneTypes.length]
      const newStone = generateStone(0, 0, stoneType)
      ...
      switch (pattern) {
        ...
        default: // Padrão aleatório
          x = Math.random() * (width - newStone.size) + newStone.size / 2
          y = Math.random() * (height - newStone.size) + newStone.size / 2
      }

      // Garante que as pedras estão dentro dos limites da caixa
      x = Math.max(newStone.size / 2, Math.min(width - newStone.size / 2, x))
      y = Math.max(newStone.size / 2, Math.min(height - newStone.size / 2, y))
